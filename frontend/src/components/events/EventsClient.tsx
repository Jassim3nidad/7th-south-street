...
export default function EventsClient({ events }: { events: any[] }) {
  ...
  return (
    <main className="site-shell">
      {/* Header */}
      <header className="page-header site-container neo-surface-sm">
                      ...
                      <div className="lg:col-span-2">
                        {event.poster_url && (
                          <div className="mb-6 relative aspect-[21/9] w-full overflow-hidden neo-surface-sm">
                            <Image src={event.poster_url} alt={event.title} fill sizes="(max-width: 1024px) 100vw, 66vw" className="object-cover opacity-90" />
                          </div>
                        )}
                        ...
                        <div className="mt-6 flex gap-3">
                          <Link href={`/events/${event.slug}`} className="btn-primary text-xs px-6 py-3 min-h-[44px] flex items-center justify-center focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#C9A96E] focus-visible:outline-offset-2">RSVP</Link>
                          <Link href={`/events/${event.slug}`} className="btn-ghost text-xs px-6 py-3 min-h-[44px] flex items-center justify-center focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#C9A96E] focus-visible:outline-offset-2">Details</Link>
                        </div>
                      </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {past.map((event: any, i: number) => (
                <motion.div key={event.id} initial={false} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                  <Link href={`/events/${event.slug}`} className="block neo-surface-sm p-5 opacity-70 hover:opacity-100 transition-opacity focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#C9A96E] focus-visible:outline-offset-2 rounded-lg">
