...
export default function AdminEventsPage() {
  ...
  const [form, setForm] = useState<any>({ title:'', description:'', event_date:'', end_date:'', location_name:'', location_address:'', max_rsvp:'0', status:'upcoming', is_featured:false })
  const [editing, setEditing] = useState<any>(null)
  const [viewingRsvp, setViewingRsvp] = useState<any>(null)
      ...
      <div className="space-y-3">
        {loading ? [...Array(3)].map((_,i) => <div key={i} className="h-20 skeleton" />) :
          events.map((ev: any) => (
              ...
              <div className="admin-event-row flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div>
                ...
                <div className="admin-event-actions flex items-center gap-4 flex-shrink-0 mt-4 md:mt-0 pt-4 md:pt-0 border-t border-white/5 md:border-t-0 w-full md:w-auto">
                  <button onClick={() => setViewingRsvp(ev)} className="bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded text-white/50 hover:text-white text-xs transition-colors border border-white/5">
                    {ev.rsvp_count} RSVPs
                  </button>
                  <button onClick={() => openEdit(ev)} className="admin-action-button text-white/30 hover:text-[#C9A96E] text-[10px] tracking-widest uppercase transition-colors ml-auto md:ml-0">Edit</button>
                  <button onClick={() => handleDelete(ev.id)} className="admin-action-button text-white/20 hover:text-[#E63B2E] text-[10px] tracking-widest uppercase transition-colors">Del</button>
                </div>
              </div>
            </div>
          ))
          ...
          <div className="theme-backdrop fixed inset-0 z-50 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div ref={modalRef} initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              ...
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setShowForm(false)} className="modal-close-button text-white/30 hover:text-white" aria-label="Close event form">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  <div><label className="text-white/30 text-[10px] tracking-widest uppercase block mb-1.5">Start Date/Time *</label>
                    <input type="datetime-local" value={form.event_date} onChange={e => setForm({...form,event_date:e.target.value})} required className="input-dark [color-scheme:dark]" /></div>
                  <div><label className="text-white/30 text-[10px] tracking-widest uppercase block mb-1.5">End Date/Time</label>
                    <input type="datetime-local" value={form.end_date} onChange={e => setForm({...form,end_date:e.target.value})} className="input-dark [color-scheme:dark]" /></div>
                </div>
                ...
                <div className="flex gap-3 pt-4">
                  <button type="submit" disabled={!form.title || !form.event_date} className="flex-1 btn-primary text-xs py-3 disabled:opacity-50">{editing ? 'Save' : 'Create'}</button>
