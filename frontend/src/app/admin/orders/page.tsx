    ...
    <div className="p-8 lg:p-10">
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          <h1 className="text-white text-3xl font-light" style={{ fontFamily: 'Cormorant Garamond, serif' }}>Orders</h1>
        </div>
        <select value={filter} onChange={e => setFilter(e.target.value)}
          className="bg-white/[0.04] border border-white/10 px-3 py-2 text-white/60 text-xs focus:outline-none focus:border-[#C9A96E]/40">
        ...
        <table className="w-full text-sm whitespace-nowrap">
          ...
          <tbody>
            {loading ? [...Array(8)].map((_, i) => (
              ...
              <tr key={o.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] cursor-pointer" onClick={() => setSelected(o)}>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-[#0E0C0A] border border-white/[0.08] w-full max-w-md p-8 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
